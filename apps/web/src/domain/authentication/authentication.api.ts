...
export class AuthenticationApi {
  static login(values: { email: string; password: string }): Promise<{ token?: string }> {
  ...
  static registerGuest(): Promise<{ token?: string }> {
    // Here you can define the logic to register a guest user
    // For example, you can generate a temporary user account or assign a role to the guest
    // This implementation depends on your specific requirements
    return Promise.resolve({ token: 'guest_token' });
  }
  ...
  static logout(document: Document): Promise<void> {
    return HttpService.api
      ...
      .catch(() => {})
      .then(() => {
        document.cookie = 'APP_ACCESS_TOKEN= ; expires = Thu, 01 Jan 1970 00:00:00 GMT';
      });
  }

  static sendResetPassword(email: string): Promise<void> {
    return HttpService.api.post(`/v1/authentication/reset-password-email`, { email });
  }

  static resetPassword(token: string, password: string): Promise<void> {
    return HttpService.api.patch(`/v1/authentication/reset-password`, { token, password });
  }

  static googleCallback(values: { credential?: string }): Promise<{ token?: string }> {
